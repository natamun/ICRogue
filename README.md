...
Built in **2022** for the **CS-107** course (EPFL), on top of a 2D grid-based game engine provided by the course. The mandatory part of the assignment (4 steps) had to be implemented on top of that engine: a player character, items, projectiles, multi-room level exploration, connectors/doors, enemies and win/lose conditions, and procedural level generation. A 5th, open-ended "extensions" step was then used to go further: new enemies (the Dark Lord and his flame skulls), new rooms, two weapons (fire/water staves), a friendly NPC, a health/heart system, and menu screens. I completed the whole project on my own.
...
<p align="center"><img src="screenshots/spawnRoom.png" width="300" alt="The player in the spawn room"><br><sub>The spawn room, where every run starts. Player health (hearts, top-left) and the equipped weapon (icon, top-right) are always on screen.</sub></p>
...
`Level0` can build its map in two different ways: two small **fixed maps** (`classic`, the intended way to experience the game, and `debug`, a minimal map used during development), or full **random generation**, which is the part of the project I'm most proud of:
